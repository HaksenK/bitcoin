...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    // Fill in header
    pblock->hashPrevBlock  = pindexPrev->GetBlockHash(true);
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, CBlockIndex* indexPrev)
{
    int64_t nTimeStart = GetTimeMicros();
    ...
    LOCK2(cs_main, m_mempool.cs);
    CBlockIndex* pindexPrev = indexPrev; // the only difference between the original method
    assert(pindexPrev != nullptr);
    nHeight = pindexPrev->nHeight + 1;
    ...
    BlockValidationState state;
    if (!TestBlockValidity(state, chainparams, *pblock, pindexPrev, false, false)) {
        throw std::runtime_error(strprintf("%s: TestlockValidity failed: %s", __func__, state.ToString()));
    }
    int64_t nTime2 = GetTimeMicros();
